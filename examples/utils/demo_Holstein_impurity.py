...
def merge_two_dicts(x, y):
    ...
    return z
#----------------------------------------------------------------------
def GenSysParams(Hopping,El_Ph_g,Phfreq_w0,Eps0_MF,Eps1,Beta):
...
def RunHolstein(sysparams,exciteparams,solverparams,outparams,approx_sigma,output_file,
    ...
    dt = solverparams['dt']
    flin = runpath+'/inp/Hol_imp_Nt{}_Ntau{}_dt{}.inp'.format(Nt,Ntau,dt)
    
    GenInputFile(flin,sysparams,exciteparams,solverparams,outparams)
    if approx_sigma == 'sMig':
        prog = runpath + 'exe/Holstein_impurity_singlebath_Migdal.x'
    elif approx_sigma == 'uMig':
        prog = runpath + 'exe/Holstein_impurity_singlebath_uMig.x'
    else:
...
if __name__ == '__main__':
    ...
    eps1=-0.5
    beta=2.0
    sysparams = GenSysParams(thop,g,w0,eps0_mf,eps1,beta)
    ...
    for approx in sigma_approx:
        output_file = './out/Hol_imp_' + approx
        RunHolstein(sysparams,exciteparams,solverparams,outparams,approx,output_file)
        
        #Plotting observables
        data = np.loadtxt(output_file + '_obs.dat')
        t = data[:,0]
        ax[0].plot(t,data[:,1],label=approx+r':$n_0$')
        ax[0].plot(t,data[:,3]+data[:,1],label=approx+r':$n_{\rm tot}$')
        ax[1].plot(t,data[:,5],label=approx+r':$X$')
        ax[1].plot(t,data[:,6],label=approx+r':$P$')
        ax[2].plot(t,data[:,8],label=approx+r'$:E_{\rm kin}$')
        ax[2].plot(t,data[:,9]+data[:,10],label=approx+r'$:E_{\rm nx}$')
        ax[2].plot(t,data[:,11]+data[:,12],label=approx+r'$:E_{\rm ph}$')
        ax[2].plot(t,data[:,13],label=approx+r'$:E_{\rm tot}$')
        ...
        t_rel,G_ret,G_les = read_gf_ret_let_tavtrel(output_file + '_green.h5','G00_tavrel',dt,tstp_plot)
