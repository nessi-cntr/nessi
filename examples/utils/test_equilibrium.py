...
if __name__ == '__main__':
    ...
    logn,err1,err2 = np.loadtxt(output_file,unpack=True)

    ax.scatter(logn,err1,label='Fourier',marker='o',s=100,edgecolor='k',facecolor='lightgray')
    ax.scatter(logn,err2,label='fix point',marker='s',s=100,edgecolor='r',facecolor='mistyrose')
    
    slope, intercept, r_value, p_value, std_err = stats.linregress(logn[6:],err1[6:])
    print('order(Fourier) : ' + '%10.2f' % slope)
    ax.plot(logn,slope*logn+intercept,'k--',label=r'$\sim \mathcal{O}(h_\tau^{' + '%10.2f' % -slope + '})$')
    
    slope, intercept, r_value, p_value, std_err = stats.linregress(logn[6:],err2[6:])
