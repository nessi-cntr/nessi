...
double InteractionEnergy(int tstp,cdmatrix &eps0, CFUNC &eps_mf, GREEN &G, GREEN &Sigma,
  ...
  int main(int argc,char *argv[]){
    ...
    double npart,err;
    cdmatrix eps0,DensM;
    GREEN G,Sigma,Pol,PxU,UxP,Chi,gtemp;
    CFUNC Ut,eps_mf;
    //..................................................
    //                timer
    //..................................................
    ...
    print_line_star(60);
    cout << "   Test program: Hubbard chain in GW approximation" << endl;
    print_line_star(60);
    ...
    try{
      ...
      //============================================================================
      {
        ...
        size_t mem_1time=0, mem_timestep=0, mem_2time=0;

        mem_1time += cntr::mem_function<double>(Nt,Nsites); // Ut
        mem_1time += cntr::mem_function<double>(Nt,Nsites); // eps_mf

        mem_timestep += cntr::mem_herm_matrix_timestep<double>(Nt,Ntau,Nsites); // deltaW

        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // G
        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // Sigma
        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // Pol
        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // Chi
        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // PxU
        mem_2time += cntr::mem_herm_matrix<double>(Nt,Ntau,Nsites); // UxP

        // convert to MB
        mem_1time = ceil(mem_1time/(double)size_MB);
        mem_timestep = ceil(mem_1time/(double)size_MB);
        mem_2time = ceil(mem_2time/(double)size_MB);

        cout << "Hamiltonian : " << mem_1time << " MB" << endl;
        cout << "time steps : " << mem_timestep << " MB" << endl;
      ...
      { // begin Matsubara Dyson iteration
        ...
        for(int iter=0;iter<=MatsMaxIter;iter++){
          ...
          // update polarization
          hubb::Polarization(tstp, G, Pol);

          // update screened interaction
          hubb::GenChi(tstp, dt, beta, Pol, Ut, PxU, UxP, Chi, SolveOrder);

          // update self-energy
          hubb::Sigma_GW(tstp, G, Ut, Chi, Sigma);
      ...
      { // begin bootstrapping
        ...
        for (int iter = 0; iter <= BootstrapMaxIter; iter++) {
          ...
          // update screened interaction
          hubb::GenChi(dt, beta, Pol, Ut, PxU, UxP, Chi, SolveOrder);
